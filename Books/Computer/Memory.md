...
- 按地址存取方式：指令执行过程中需要访问主存时。首先由CPU将被访问单元的地址送到MAR，经地址总线送至主存的地址寄存器，以便地址译码器译码选中的单元。同时CPU将读写信号通过控制线送到主存的读写控制电路。
...
<img src="../../pictures/计算机组成原理-静态RAM的基本单元电路.drawio.svg" width="800"/> 
...
<img src="../../pictures/计算机组成原理-SRAM写周期时序.drawio.svg" width="600"/> 
...
<img src="../../pictures/20231203144420.png" width="700"/> 

###### 三管MOS

<img src="../../pictures/20231203110014.png" width="350"/>  
...
<img src="../../pictures/20231203145309.png" width="600"/> 
...
<img src="../../pictures/20231203150511.png" width="500"/> 
...
- EPROM（可擦除可编程只读存储器）：由用户通过编程器写入自己的程序，可对其内容多次改写，但次数有限，且写入时间长。 
...
<img src="../../pictures/计算机组成原理-线选法.drawio.svg" width="450"/> 
...
<img src="../../pictures/20231203184425.png" width="400"/> 
...
<img src="../../pictures/20231203190438.png" width="500"/> 
...
- 芯片内的数据输出路径（由SRAM到I/O）与数据输入路径（由I/O到读放大器和列写选择）是分开的，允许在写操作完成的同时启动同一行的读操作。此外，在SRAM读出期间可同时对DRAM阵列进行刷新。
...
<img src="../../pictures/计算机组成原理-二路组相联映射的标记阵列.drawio.svg" width="380"/> 
...
<details>
    ...
    （1.1 采用直接映射方式）4256位<br/>
    按字节编址，主存地址空间为256MB（2<sup>28</sup>B），为28位。<br/>
    块内地址取决于Cache行长64B（2<sup>6</sup>B），即6位。<br/>
    行号取决于Cache行数8个（2<sup>3</sup>），即3位。<br/>
    若不考虑用于Cache的一致维护性和替换算法控制位，并且采用直接映射方式，则以上剩余的字段为标记字段 28-6-3=19位。<br/>
    一个Cache行存储的信息量取决于行长64B，即512 bit。<br/>
    <img src="../../pictures/计算机组成原理-Cache行标记字段1.drawio.svg" width="300"/><br/>
    综上，该数据Cache的容量为 8*(1+19+512) = 4256 bit<br/>
    （2.1 采用直接映射方式）2<br/>
    直接映射方式中，主存按照块的大小划分，主存地址(3200)<sub>10</sub>对应的字块号为3200B/64B=50<br/>
    而Cache只有8行， Cache行号 = 主存块号 mod Cache总行数 = 50 mod 8 = 2<br/>
    （2.2 采用二路组相联映射方式）4或5<br/>
    二路相联映射，实际上将两个Cache行合并，组数Q=8/2=4，则Cache组号 = 主存块号 mod Cache组数（Q）= 50 mod 4 = 2，对应的行号为4或5。<br/>
    （3.1 采用直接映射方式）
</details>
<details>
    <summary>某存储系统中，主存容量是Cache的4096倍，Cache被分为64个块，当主存地址和Cache地址采用直接映像方式时，地址映射表的大小应为_____。</summary>
    Cache采用直接映射方式，主存块号 = 标记 + Cache行号，且Cache行号 = 主存块号（对应主存容量） mod Cache总行数（对应Cache容量），而主存块的大小和Cache块的大小相等，且主存容量 / Cache容量 = 4096（2<sup>12</sup>），得到标记位为12bit（主存号的低12位）。而直接映射方式的Cache块地址 = 有效位 + 标记位 = 13bit，故地址映射表的大小 = 13bit * 64。
</details>
...
<details>
    <summary>某计算机主存地址空间大小为256MB，按字节编址。虚拟地址空间大小为4GB，采用页式存储管理，页面大小为4KB，TLB采用全相联映射，有4个页表项，内容如表，则对虚拟地址03FF F180H进行虚实地址变换的结果是______。<br/>
        <table>
    ...
    页式虚拟存储器的虚拟地址 = 虚页号 + 页内地址（页偏移），其中，按字节编址，则页内地址位数 = 12（4KB 2<sup>12</sup>B）、虚页号位数 = 32 - 12  = 20（4GB 2<sup>32</sup>B）。故虚地址03FF F180H对应的标记为前20位，即03FFFH，查找对应页表中的页表项，得到页框号（实页号）0153H。拼接页框号和页内地址后，得到实地址 = 015 3180H。
</details>
...
- 每个TLB项 = TLB标记字段 + 页表项，TLB标记表示该表取自页表中哪个虚页号对应的页表项，（1）全相联映射方式下的TLB标记对应页表项的虚页号；（2）组相联映射方式下的TLB标记对应虚页号的高位，而虚页号的低位用于选择TLB组的组索引。
...
<img src="../../pictures/计算机组成原理-TLB虚拟存储器的CPU访存过程.drawio.svg" width="800"/> 
...
<img src="../../pictures/计算机组成原理-段式虚拟存储器的地址变换过程.drawio.svg" width="500"/> 
