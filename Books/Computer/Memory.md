...
- Cache行中的信息是主存中某个块的副本，地址映射将主存地址空间映射到Cache地址空间，将存放在主存中的信息按照某种规则装入Cache。

1. 为每个块加上一个标记（相当于主存中块的编号），指明其对应主存的哪一块的副本。
2. 每个Cache行需要一个有效位来说明Cache行中信息是否有效。
...
直接映射的关系函数： Cache行号 = 主存块号（主存地址空间容量对应的位数） mod Cache总行数
