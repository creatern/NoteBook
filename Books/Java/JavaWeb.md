...
2. 类样式表 class属性 `.类名`
3. ID样式表 id属性 `#ID名`
   - 每个HTML页面中id属性不能重复，因此ID样式表最多只能再HTML中使用一次。
...
![](C:/Users/zjk10/OneDrive/NoteBook/pictures/Snipaste_2023-03-12_19-44-49.jpg =800x)

### 调用事件处理

#### JavaScript

- 在JavaScript中调用事件处理，需要获得要处理对象的引用，再将要执行的处理还是赋值给对应的事件。

#### HTML

- 在HTML中分配事件处理程序，只需要在HTML标记中添加相应的事件，并在其中指定要执行的代码或函数即可。
