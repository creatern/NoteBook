...
public class AdjacencyList {
    ...
    class Node {
        ...
        private int weight = Integer.MAX_VALUE; //权重
        private char name;
    ...
    public void init(int nodeNumber) {
        ...
        list = new Node[nodeNumber];
        //初始化链表数组
        for (int i = 0; i < list.length; i++) {
            System.out.println("请输入第" + (i + 1) + "个顶点的名称：");
            list[i] = new Node(scanner.next().charAt(0));
        }
        //添加顶点的边
        for (int i = 0; i < list.length; i++) {
            int number = 0;
            while (number != list.length) {
                System.out.println("是否添加第" + (i + 1) + "个顶点的边：(y/n)");
                char flag = scanner.next().charAt(0);
                if (flag == 'n') {
                    break;
                }
                System.out.println("请输入连接的顶点的名称：");
                char nodeName = scanner.next().charAt(0);
                System.out.println("请输入该边的权值：");
                int nodeWeight = scanner.nextInt();
                list[i].next = new Node(nodeWeight, nodeName);
                number++;
            }
        }
    }
    ...
    public void print() {
        for (int i = 0; i < list.length; i++) {
            Node temp = list[i];
            while (temp != null) {
                System.out.print(list[i].name + "--" + temp.next.weight + "-->" + temp.next.name);
                temp = temp.next;
                System.out.println();
            }
        }
