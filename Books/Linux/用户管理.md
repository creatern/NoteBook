...
-  Linux 是一个多用户的操作系统（允许多个用户通过不同主机或者终端访问一个独立系统）
...
| 用户相关注释 |                                                           |
| 用户的主目录 |                                                           |
| 默认shell    | 用户登录系统时运行的shell，通常为/bin/bash                |
...
- <span name="useradd"><code>useradd</code></span>或<code>adduser</code>：添加用户，自动完成用户信息、基本组、主目录等的创建工作（主要根据[/etc/default/useradd](#/etc/default/useradd)的配置），并在创建过程中对用户初始信息进行定制，并不包括用户密码的设置（[passwd](#passwd)）
...
4. <b>通过sudo使用的命令可能会切换到root用户的环境下执行</b>！&#128163;如果要通过sudo命令执行rm、find等命令，必须使用绝对路径，绝不能使用相对路径。
...
1. `who`返回的是在线用户以及登录时间、登录的IP；
2. `w`返回的是在线用户、登录的地址IP，登录时间、在线时间
...
## [默认shell](./Shell.md#bashrc)（<code>$HOME</code>的启动文件）