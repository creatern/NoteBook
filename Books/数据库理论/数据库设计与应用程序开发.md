...
- 令r(R)为一个关系模式，并令&alpha;&sube;R且&beta;&sube;R。多值依赖&alpha;&rarr;&rarr;&beta;在R上成立的条件是：在关系r(R)的任意合法实例中，对于r中满足t<sub>1</sub>[a]=t<sub>2</sub>的所有元组对t<sub>1</sub>和t<sub>2</sub>，在r中都存在元组t<sub>3</sub>和t<sub>4</sub>，使得：
    - t<sub>1</sub>[&alpha;]=t<sub>2</sub>[&alpha;]=t<sub>3</sub>
    - [&alpha;]=t<sub>4</sub>[&alpha;]
...
<table>
    ...
    <tr>
        <th>t<sub>1</sub></th>
        <td>&alpha;<sub>1</sub> ... &alpha;<sub>i</sub></td>
        <td>&alpha;<sub>i+1</sub> ... &alpha;<sub>j</sub></td>
        <td>&alpha;<sub>j+1</sub> ... &alpha;<sub>n</sub></td>
    </tr>
    <tr>
        <th>t<sub>2</sub></th>
        <td>&alpha;<sub>1</sub> ... &alpha;<sub>i</sub></td>
        <td>&alpha;<sub>i+1</sub> ... &alpha;<sub>j</sub></td>
        <td>&beta;<sub>j+1</sub> ... &beta;<sub>n</sub></td>
    </tr>
    <tr>
        <th>t<sub>3</sub></th>
        <td>&alpha;<sub>1</sub> ... &alpha;<sub>i</sub></td>
        <td>&beta;<sub>i+1</sub> ... &beta;<sub>j</sub></td>
        <td>&alpha;<sub>j+1</sub> ... &alpha;<sub>n</sub></td>
    </tr>
</table>
...
- 一个关系模式R是关于一个函数依赖和多值依赖的集合D的第四范式的条件是：对于D<sup>+</sup>中所有形如&alpha;&rarr;&rarr;的多值依赖（其中，&alpha;&sube;R且&beta;&sube;R），至少有以下之一成立：
...
2. 所有形如&alpha;&rarr;&rarr;&cap;R<sub>i</sub>的多值依赖，其中&alpha;&sibe;R<sub>i</sub>且&alpha;&rarr;&rarr;属于D<sup>+</sup>。
...
- 时态外码（temporal foreign-key）不仅要确保引用关系中的每个元组（如r）在被引用关系中具有匹配的元组（如s），而且还要考虑它们的时间区间。当并不要求s中的匹配元组具有完全相同的时间间隔，不要求s中的单个元组具有包含r时间区间的时间区间；允许r元组的时间区间被一个或多个s元组覆盖。
