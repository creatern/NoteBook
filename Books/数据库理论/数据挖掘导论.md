...
<table>
    ...
    <tr>
        <td>r=&infin;</td>
        <td>L<sub>&infin;</sub>范式、L<sub>max</sub>范式、上确界距离</td>
    </tr>
</table>
...
- 给定一种情况，未被顾客购买的商品数（0，<code>f<sub>00</sub></code>）远大于被其购买的商品数（1）。该情况下，导致SMC这样的相似性度量将会判定所有的事务都是类似的（都未被顾客购买），而这是不应该的。因此，需要使用剔除了不敢兴趣的干扰的Jaccard系数来处理。
