...
<table>
    ...
    <tr>
        ...
        <td>$$frenquency(v_i)=\frac{具有属性值v_i的对象数}{m}$$</td>
    </tr>
    <tr>
        <td>众数 mode</td>
        <td>具有最高频率的值</td>
    ...
    <tr>
        ...
        <td>对于离群值很敏感<br/>$$mean(x)=\hat{x}=\frac{1}{m}\sum^m_{i=1}x_i$$</td>
    </tr>
    <tr>
        <td>中位数 median</td>
        <td>$$median(x)=\begin{cases}x_{(r+1)} \ 如果m是奇数，即m=2r+
1 \\ \frac{1}{2}(x_{(r)}+x{(r+1)} \ 如果m是偶数，即m=2r)\end{cases}$$</td>
    </tr>
    <tr>
        <td>极差 range</td>
        <td rowspan="5">散布度量<br>连续数据</td>
        <td>$$range(x)=max(x)-min(x)=x_{(m)}-x_{(1)}$$</td>
    </tr>
    <tr>
        <td>方差 variance<br/>标准差 standard deviation</td>
        <td>对离群点特别敏感<br/>$$variance(x)=s_x^2=\frac{1}{m-1}\sum^m_{i=1}(x_i-\hat{x})^2$$</td>
    </tr>
    <tr>
        <td>绝对平均偏差 AAD</td>
        <td>absolute deviation<br/>$$AAD(x)=\frac{1}{m}\sum^m_{i=1}|x_i-\hat{x}|$$</td>
    </tr>
    <tr>
        <td>中位数绝对偏差 MAD</td>
        <td>median absolute deviation<br/>$$MAD(x)=median({|x_1-\hat{x}|,...,|x_m-\hat{x}|})$$</td>
    </tr>
    <tr>
        <td>四分位数极差 IQR</td>
        <td>interquartile range<br/>$$interquartile range(x)=x_{75%}-x_{25%}$$</td>
    </tr>
</table>
