...
<img src="../pictures/prim.gif" width="1400"/>
...
| 发生时间                           | 说明                                                         |
| ---------------------------------- | ------------------------------------------------------------ |
| ve[k]<br />（事件的最早发生时间 ） | 只有进入事件vk的所有活动`<vj,vk>`（使用p[k]表示该活动集）都结束，vk代表的事件才能发生；<br />所有活动`<vj,vk>`的最早结束时间为：`ve[j]+len<vj,vk>`。<br />ve[j]：上一个事件的最早发生时间 <br />len`<vj,vk>`：相应的活动（边）的权值，该活动需要的时间。<br />ve[0] = 0、ve[k] = max{ve[j] + len`<vj,vk>`}，(`<vj,vk>`∈p[k]) |
|                                    |                                                              |
